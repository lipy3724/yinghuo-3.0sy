 ...
 * @param {string} taskId - 任务ID，如果为null则尝试查找最近的任务
 * @param {string} reason - 退款原因
 * @returns {Promise<boolean>} - 退款是否成功
 */
async function refundLocalRedrawCredits(userId, taskId, reason = '任务失败') {
    try {
        console.log(`开始处理局部重绘退款: 用户ID=${userId}, 任务ID=${taskId || '未指定'}, 原因=${reason}`);
        ...
        if (!usage) {
            console.log(`未找到用户${userId}的局部重绘使用记录，无需退款`);
            return false;
        }
        ...
        if (isFree) {
            ...
            console.log(`局部重绘免费使用退款完成: 用户ID=${userId}, 回退使用次数`);
            return true;
        }
        ...
        if (creditCost > 0) {
            ...
            console.log(`局部重绘积分退款完成: 用户ID=${userId}, 退还积分=${creditCost}, 用户当前积分=${user.credits}`);
            return true;
        }
        
        console.log(`任务${task.taskId}无需退款: 积分=${creditCost}, 免费=${isFree}`);
        return false;
        
    } catch (error) {
        console.error('局部重绘退款失败:', error);
        return false;
    }
}
