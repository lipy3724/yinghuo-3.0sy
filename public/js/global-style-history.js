 ...
 * 全局风格化历史记录管理模块
 */
window.GlobalStyleHistory = (function() {
  // 私有变量
  const API_BASE = '/api/global-style-history';
   ...
   * @param {String} data.prompt 风格提示词
   * @param {Number} data.strength 风格强度
  ...
  function display(records, containerId) {
    ...
    if (!validRecords || validRecords.length === 0) {
      // 使用翻译函数获取"暂无历史记录"文本
      const noHistoryText = window.getTranslation ? window.getTranslation('global_stylization.no_history') : 'No history records';
  ...
  function createHistoryCard(record, containerId) {
    ...
    card.innerHTML = `
      <div class="p-4 pb-12">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs bg-purple-100 text-purple-800 px-2 py-1 rounded">${record.prompt || '全局风格化'}</span>
        </div>
      ...
      <div class="absolute bottom-3 right-4">
        <button class="download-history bg-purple-500 hover:bg-purple-600 text-white py-1 px-3 rounded-md flex items-center text-xs download-history-btn" data-id="${record.id}">
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', (e) => {
        ...
        // 下载图片
        downloadImage(record.resultImage, `全局风格化-${record.prompt || '风格化'}-${new Date().toISOString().slice(0,10)}.png`);
      });
    }
