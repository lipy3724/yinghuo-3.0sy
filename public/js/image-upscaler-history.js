 ...
 * 图像高清放大历史记录管理模块
 */
window.ImageUpscalerHistory = (function() {
  // 私有变量
  const API_BASE = '/api/image-upscaler-history';
   ...
   * @param {Number} data.upscaleFactor 放大倍数
  ...
  function display(records, containerId) {
    ...
    if (!validRecords || validRecords.length === 0) {
      // 使用翻译函数获取"暂无放大记录"文本
      const noHistoryText = window.translate ? window.translate('image_upscaler.no_upscale_records') : '暂无图片放大记录';
  ...
  function createHistoryCard(record, containerId) {
    ...
    card.innerHTML = `
      <div class="p-4 pb-12">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">${record.upscaleFactor}x</span>
        </div>
        <div class="flex justify-center">
          <div class="w-full max-w-xs">
            <p class="text-xs text-gray-500 mb-1 text-center">${window.translate ? window.translate('image_upscaler.result_image') : '结果图'}</p>
            <img src="${record.resultImage}" alt="${window.translate ? window.translate('image_upscaler.result_image') : '结果图'}" class="w-full h-auto rounded history-thumbnail">
          </div>
        </div>
      </div>
      <div class="absolute bottom-3 right-4">
        <button class="download-history bg-blue-500 hover:bg-blue-600 text-white py-1 px-3 rounded-md flex items-center text-xs download-history-btn" data-id="${record.id}">
          <i class="ri-download-line mr-1"></i>${window.translate ? window.translate('common.download') : 'Download'}
        </button>
      </div>
    `;
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', (e) => {
        ...
        // 下载图片
        const filename = window.translate ? 
          `${window.translate('image_upscaler.main_title')}-${record.upscaleFactor}x-${new Date().toISOString().slice(0,10)}.png` :
          `图像高清放大-${record.upscaleFactor}x-${new Date().toISOString().slice(0,10)}.png`;
        downloadImage(record.resultImage, filename);
      });
    }
