...
(function () {
  ...
  function showMobileAlert(isPageBlock = false) {
    ...
    if (!document.body) {
      ...
      if (document.readyState === 'loading') {
        ...
        // 如果readyState不是loading但body仍不存在，使用setTimeout重试
        setTimeout(function() {
          showMobileAlert(isPageBlock);
        }, 100);
      }
  ...
  function checkCurrentPage() {
    console.log('检查当前页面路径:', window.location.pathname);
    
    // 检查当前路径是否在白名单中
    if (allowList.has(window.location.pathname)) {
      console.log('当前页面在白名单中，允许访问');
      return;
    }
    
    // 检查当前路径是否是功能页面
    const isFeaturePage = isAIFunctionLink(window.location.pathname);
    console.log('当前页面是否是功能页面:', isFeaturePage);
    
    if (isFeaturePage) {
      // 如果是功能页面，显示弹窗并阻止进一步加载
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      console.log('点击元素:', link.tagName, '链接:', href || dataHref || '无链接');
      ...
      if (dataHref && isAIFunctionLink(dataHref)) {
        console.log('拦截data-href功能链接点击:', dataHref);
        e.preventDefault();
        e.stopPropagation();
        showMobileAlert();
        return false;
      ...
      if (onclick) {
        if (onclick.includes('location.href') || onclick.includes('window.location') || 
          ...
          if (urlMatch && urlMatch[2] && isAIFunctionLink(urlMatch[2])) {
            console.log('拦截onclick中的跳转:', urlMatch[2]);
            e.preventDefault();
            e.stopPropagation();
            showMobileAlert();
            return false;
        ...
        if (onclick.includes('checkAuthAndRedirect')) {
          ...
          if (urlMatch && urlMatch[1] && isAIFunctionLink(urlMatch[1])) {
            console.log('拦截checkAuthAndRedirect调用:', urlMatch[1]);
            e.preventDefault();
            e.stopPropagation();
            showMobileAlert();
            return false;
  ...
  function overrideGlobalFunctions() {
    ...
    window.open = function(url, name, specs) {
      console.log('window.open被调用:', url);
      if (url && isAIFunctionLink(url)) {
        showMobileAlert();
        return null;
      }
