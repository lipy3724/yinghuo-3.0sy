 ...
 */
async function uploadImageToOSS(userId, imageData, prefix = 'diantu') {
    try {
        if (!imageData) {
 ...
 */
async function saveDiantuHistoryOSS(userId, historyItem) {
    try {
        logger.info('保存垫图历史记录到OSS', {
            userId,
        ...
        // 确保必要的字段存在
        if (!historyItem || !historyItem.processedImageUrl) {
        ...
        if (historyItem.processedImageUrl) {
            try {
                ossHistoryItem.processedImageUrl = await uploadImageToOSS(
                    userId,
                    historyItem.processedImageUrl,
                    'diantu/results'
                );
        ...
        // 上传原始图片（如果有）
        if (historyItem.originalImageUrl) {
            try {
                ossHistoryItem.originalImageUrl = await uploadImageToOSS(
                    userId,
                ...
                // 原始图片上传失败不影响整体流程
                ossHistoryItem.originalImageUrl = null;
            }
        }
        
        if (!ossHistoryItem.processedImageUrl) {
            logger.error('保存失败: OSS上传后的结果图片URL为空', { userId });
            throw new Error('OSS上传后的结果图片URL为空');
        }
        
        // 添加创建时间
        if (!ossHistoryItem.processTime) {
            ossHistoryItem.processTime = new Date().toISOString();
        }
        
        // 确保有处理类型
        if (!ossHistoryItem.processType) {
            ossHistoryItem.processType = '垫图';
        }
        ...
        // 保存元数据到OSS
        const metadataKey = `diantu/metadata/${userId}/${recordId}.json`;
 ...
 */
async function getDiantuHistoryOSS(userId, options = {}) {
    try {
        const { limit = 10, last24Hours = false } = options;
        
        logger.info('获取垫图历史记录从OSS', {
            userId,
            limit,
            last24Hours
        });
        ...
        // 列出元数据文件
        const metadataPrefix = `diantu/metadata/${userId}/`;
        ...
        for (const file of metadataFiles) {
            ...
            try {
                ...
                // 检查时间（如果需要）
                if (last24Hours && historyItem.processTime) {
                    const itemDate = new Date(historyItem.processTime);
 ...
 */
async function clearDiantuHistoryOSS(userId) {
    try {
        logger.info('清空垫图历史记录（OSS）', { userId });
...
module.exports = {
    saveDiantuHistoryOSS,
    getDiantuHistoryOSS,
    clearDiantuHistoryOSS,
    uploadImageToOSS
};
