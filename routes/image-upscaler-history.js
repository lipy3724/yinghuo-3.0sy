...
/**
 * @route   GET /api/image-upscaler-history/list
 * @desc    获取图像高清放大历史记录列表
 * @access  Private
 */
router.get('/list', protect, async (req, res) => {
  try {
    ...
    console.log(`获取用户 ${userId} 的图像高清放大历史记录`);
    ...
    console.error('获取图像高清放大历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/image-upscaler-history/save
 * @desc    保存图像高清放大历史记录
 * @access  Private
 */
router.post('/save', protect, async (req, res) => {
  try {
    const userId = req.user.id;
    const { originalImage, resultImage, upscaleFactor, metadata } = req.body;
    ...
    console.error('保存图像高清放大历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   DELETE /api/image-upscaler-history/:id
 * @desc    删除图像高清放大历史记录
 * @access  Private
 */
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    console.error('删除图像高清放大历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/image-upscaler-history/clear
 * @desc    清空图像高清放大历史记录
 * @access  Private
 */
router.post('/clear', protect, async (req, res) => {
  try {
    ...
    console.error('清空图像高清放大历史记录失败:', error);
    res.status(500).json({
      success: false,
