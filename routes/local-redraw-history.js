...
/**
 * @route   GET /api/local-redraw-history/list
 * @desc    获取局部重绘历史记录列表
 * @access  Private
 */
router.get('/list', protect, async (req, res) => {
  try {
    ...
    console.log(`获取用户 ${userId} 的局部重绘历史记录`);
    ...
    console.error('获取局部重绘历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/local-redraw-history/save
 * @desc    保存局部重绘历史记录
 * @access  Private
 */
router.post('/save', protect, async (req, res) => {
  try {
    // 使用当前登录用户的ID
    const userId = req.user.id;
    const { originalImage, resultImage, prompt, maskImage, metadata } = req.body;
    ...
    console.error('保存局部重绘历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   DELETE /api/local-redraw-history/:id
 * @desc    删除局部重绘历史记录
 * @access  Private
 */
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    console.error('删除局部重绘历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/local-redraw-history/clear
 * @desc    清空局部重绘历史记录
 * @access  Private
 */
router.post('/clear', protect, async (req, res) => {
  try {
    ...
    console.error('清空局部重绘历史记录失败:', error);
    res.status(500).json({
      success: false,
...
async function loadHistoryFromOSS(userId) {
  try {
    const ossPath = `local-redraw/history/${userId}/records.json`;
