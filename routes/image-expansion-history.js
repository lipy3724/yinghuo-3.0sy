...
/**
 * @route   GET /api/image-expansion-history/list
 * @desc    获取智能扩图历史记录列表
 * @access  Private
 */
router.get('/list', protect, async (req, res) => {
  try {
    ...
    console.log(`获取用户 ${userId} 的智能扩图历史记录`);
    ...
    console.error('获取智能扩图历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/image-expansion-history/save
 * @desc    保存智能扩图历史记录
 * @access  Private
 */
router.post('/save', protect, async (req, res) => {
  try {
    const userId = req.user.id;
    const { originalImage, resultImage, prompt, metadata, creditUsed } = req.body;
    ...
    const historyRecord = {
      ...
      prompt: prompt || '',
      metadata: metadata || {},
      creditUsed: creditUsed || 7, // 默认使用7积分
    ...
    console.error('保存智能扩图历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   DELETE /api/image-expansion-history/:id
 * @desc    删除智能扩图历史记录
 * @access  Private
 */
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    console.error('删除智能扩图历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/image-expansion-history/clear
 * @desc    清空智能扩图历史记录
 * @access  Private
 */
router.post('/clear', protect, async (req, res) => {
  try {
    ...
    console.error('清空智能扩图历史记录失败:', error);
    res.status(500).json({
      success: false,
...
async function loadHistoryFromOSS(userId) {
  try {
    const ossPath = `image-expansion/history/${userId}/records.json`;
