...
/**
 * @route   GET /api/global-style-history/list
 * @desc    获取全局风格化历史记录列表
 * @access  Private
 */
router.get('/list', protect, async (req, res) => {
  try {
    ...
    console.log(`获取用户 ${userId} 的全局风格化历史记录`);
...
/**
 * @route   POST /api/global-style-history/save
 * @desc    保存全局风格化历史记录
 * @access  Private
 */
router.post('/save', protect, async (req, res) => {
  try {
    const userId = req.user.id;
    const { originalImage, resultImage, prompt, strength, metadata } = req.body;
    ...
    console.error('保存全局风格化历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   DELETE /api/global-style-history/:id
 * @desc    删除全局风格化历史记录
 * @access  Private
 */
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    console.error('删除全局风格化历史记录失败:', error);
    res.status(500).json({
      success: false,
...
/**
 * @route   POST /api/global-style-history/clear
 * @desc    清空全局风格化历史记录
 * @access  Private
 */
...
async function loadHistoryFromOSS(userId) {
  try {
    const ossPath = `global-style/history/${userId}/records.json`;
